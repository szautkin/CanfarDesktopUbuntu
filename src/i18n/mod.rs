...
static HAND_PAIRS: &[(&str, &str)] = &[
    ...
    ("Advertise the catalog and foundational tools instead of all of them, and send the rest as a grouped map. Every tool stays callable; agents that ask for a tool by name are unaffected.",
     "Annoncer les outils du catalogue et les outils fondamentaux plutôt que tous, et envoyer le reste sous forme de carte groupée. Tous les outils restent appelables ; les agents qui demandent un outil par son nom ne sont pas affectés."),
