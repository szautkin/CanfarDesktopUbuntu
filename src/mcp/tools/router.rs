        ...
        match name {
            "list_pending_proposals" => {
                let pending: Vec<_> = self
                    ...
                    .collect();
                Some(ToolResult::Data(serde_json::json!({ "proposals": pending })))
            }
            "get_proposal_state" => {
            ...
            "withdraw_proposal" => {
                ...
                let owned = self.proposals.get(id).map(|p| visible(&p.origin)).unwrap_or(false);
                if !owned {
            ...
            "list_events" => {
                ...
                let lost = cursor > 0
                    && log.oldest_seq().map(|o| o > cursor + 1).unwrap_or(false);
...
impl ToolRouter for McpToolRouter {
    ...
    fn dispatch<'a>(
        ...
        Box::pin(async move {
            ...
            let mut family_result = super::family_dispatch(name, &self.services, &args, &self.proposals).await;
            if family_result.is_none() {
                family_result = write::dispatch(name, &self.services, &args, &self.proposals).await;
            }
            if let Some(result) = family_result {
                ...
                if let ToolResult::Proposed(p) = &result {
                    let origin =
                        ctx.is_external().then(|| ctx.client_label().to_string());
                    ...
                    if !p.destructive && auto_apply_on {
                        ...
                        if self.proposals.claim(&p.id).is_some() {
                            match super::apply_any(&self.services, p).await {
                                Ok(msg) => {
                                    self.proposals
                                        .settle(&p.id, super::proposals::ProposalState::Applied);
                                ...
                                Err(e) => {
                                    self.proposals
                                        .settle(&p.id, super::proposals::ProposalState::Rejected);
