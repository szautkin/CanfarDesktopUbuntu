...
use crate::models::tool_category as catalog;
...
fn categorize(descriptors: Vec<ToolDescriptor>) -> Vec<CatData> {
    // The grouping and its order come from the taxonomy, which the MCP catalog
    // reads too. Two loops that agree today are two loops that disagree later.
    catalog::group_by_category(descriptors, |d| d.name.clone())
        .into_iter()
        .map(|(cat, tools)| CatData {
            id: cat.id,
            title: cat.title,
            icon: cat.icon,
            summary: cat.summary,
